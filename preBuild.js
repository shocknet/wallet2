...
fs.readFile(androidStringsXmlPath, 'utf-8', (readErr, data) => {
  ...
  xml2js.parseString(data, (parseErr, result) => {
    ...
    // Check if the app_url string exists
    result.resources.string.push({ $: { name: 'app_name' }, _: process.env.APP_NAME });
    result.resources.string.push({ $: { name: 'title_activity_main' }, _: process.env.APP_NAME });
    const existingAppUrl = result.resources.string.find((item) => item.$.name === 'app_url');
    if (!existingAppUrl) {
      // Add the app_url string if it doesn't exist
      result.resources.string.push({ $: { name: 'app_url' }, _: process.env.VITE_APP_URL });
    } else {
      // Update the existing app_url string
      existingAppUrl._ = process.env.VITE_APP_URL;
    }
