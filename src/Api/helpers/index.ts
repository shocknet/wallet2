...
export const getNostrBtcAddress = async (pasteField: string) => {
	const res = await (await getNostrClient(pasteField)).NewAddress({ addressType: AddressType.WITNESS_PUBKEY_HASH })
	if (res.status !== 'OK') {
		throw new Error(res.reason);
	}
