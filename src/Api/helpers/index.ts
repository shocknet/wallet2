...
import { createNofferInvoice as createNofferInvoiceFromLib } from "@/lib/noffer";
import type { OfferPointer } from "@shocknet/clink-sdk";
...
export const createNofferInvoice = async (noffer: OfferPointer, keys: NostrKeyPair, amount?: Satoshi) => {
	const res = await createNofferInvoiceFromLib(noffer, keys, amount)
	if (typeof res !== "string") {
		if (res.code === 5 && res.range) {
			throw new Error("value must be between " + res.range.min + " and " + res.range.max);
		}
