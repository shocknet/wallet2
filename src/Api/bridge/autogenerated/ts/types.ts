...
export type nip05Names = {
    ...
    clink_offer: Record<string, string>
    clink_debit: Record<string, string>
}
export const nip05NamesOptionalFields: [] = []
export type nip05NamesOptions = OptionsBaseMessage & {
    checkOptionalsAreSet?: []
    clink_debit_CustomCheck?: (v: Record<string, string>) => boolean
    names_CustomCheck?: (v: Record<string, string>) => boolean
    clink_offer_CustomCheck?: (v: Record<string, string>) => boolean
}
export const nip05NamesValidate = (o?: nip05Names, opts: nip05NamesOptions = {}, path: string = 'nip05Names::root.'): Error | null => {
    ...
    if (typeof o.clink_offer !== 'object' || o.clink_offer === null) return new Error(`${path}.clink_offer: is not an object or is null`)
    for (const key in o.clink_offer) {
        if (typeof o.clink_offer[key] !== 'string') return new Error(`${path}.clink_offer['${key}']: is not a string`)
    }

    if (typeof o.clink_debit !== 'object' || o.clink_debit === null) return new Error(`${path}.clink_debit: is not an object or is null`)
    for (const key in o.clink_debit) {
        if (typeof o.clink_debit[key] !== 'string') return new Error(`${path}.clink_debit['${key}']: is not a string`)
    }
...
export type LnurlPayInfoResponseOptionalField = 'allowsNostr' | 'nostrPubkey' | 'clink_offer' | 'nip05'
export const LnurlPayInfoResponseOptionalFields: LnurlPayInfoResponseOptionalField[] = ['allowsNostr', 'nostrPubkey', 'clink_offer', 'nip05']
export type LnurlPayInfoResponseOptions = OptionsBaseMessage & {
    ...
    clink_offer_CustomCheck?: (v?: string) => boolean
...
export const LnurlPayInfoResponseValidate = (o?: LnurlPayInfoResponse, opts: LnurlPayInfoResponseOptions = {}, path: string = 'LnurlPayInfoResponse::root.'): Error | null => {
    ...
    if ((o.clink_offer || opts.allOptionalsAreSet || opts.checkOptionalsAreSet?.includes('clink_offer')) && typeof o.clink_offer !== 'string') return new Error(`${path}.clink_offer: is not a string`)
    if (opts.clink_offer_CustomCheck && !opts.clink_offer_CustomCheck(o.clink_offer)) return new Error(`${path}.clink_offer: custom check failed`)
