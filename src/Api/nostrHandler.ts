import { nip69, Event, UnsignedEvent, Relay, finalizeEvent, nip04, Filter } from 'nostr-tools'
...
export default class RelayCluster {
    ...
    addRelays = (relaysSettings: RelaysSettings, connectedCallback: () => void, eventCallback: (event: NostrEvent) => void, disconnectCallback: (disconnectedRelayUrl: string) => void) => {
        const relayUrls = relaysSettings.relays;

        // connectCallback is called when at least one relay receives eose event
        relayUrls.forEach(r => {
            this.addRelay(r, relaysSettings.keys, connectedCallback, eventCallback, () => disconnectCallback(r))
        })
    }
    ...
    async addRelay(relayUrl: string, keys: NostrKeyPair, connectedCallback: () => void, eventCallback: (event: NostrEvent) => void, disconnectCallback: () => void) {
    ...
    foregroundReconnect = async () => {
        ...
        await Promise.allSettled(this.relays.values().map(async relay => {
            await this.getOrCreateRelay( // Recreate the relay connection, using the existing args
                ...
                {
                    connectedCallback: relay.connectedCallback,
