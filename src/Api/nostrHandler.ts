...
function parseBeaconContent(content: string): BeaconData | null {
	try {
		const data = JSON.parse(content) as BeaconData;
		if (BeaconDataValidate(data) !== null) return null;
		return data;
	} catch {
		return null;
	}
...
export const fetchBeaconDiscovery = async (pubkey: string, relays: string[]): Promise<BeaconDiscoveryResult> => {
	...
	const data = parseBeaconContent(event.content);
	if (!data) return null;
