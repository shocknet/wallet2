...
const Channels = ({ done }: { done: () => void }) => {
	...
	useEffect(() => {
		...
		fetchChannels({ pubkey: source.lpk, relays: source.relays }, source.keys)
	}, [selectedSource])
	const fetchChannels = async (nprofile: { pubkey: string, relays: string[] }, keys: NostrKeyPair) => {
	...
	return (
		<div className="Channels">
			...
			<OpenChannelComponent adminSource={selectedSource} />
			<EditChannel adminSource={selectedSource} selectedChannel={selectedChannel} deselect={() => setSelectedChannel(null)} />
			<button onClick={() => done()} className="Manage_save">
				Done
			</button>
			<div className="Channels_footer">
				Connected to <br />
				{selectedSource ? nip19.nprofileEncode({ pubkey: selectedSource.lpk, relays: selectedSource.relays }) : ""}
			</div>
		</div>
	);
};
