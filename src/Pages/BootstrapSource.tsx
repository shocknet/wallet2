			...
			<IonContent className="ion-padding ion-content-only">
				<div className="min-h-full flex flex-col gap-10 justify-center items-center w-full max-w-md mx-auto">
					<ScreenIntro
						...
						description="Choose how this profile connects to Lightning so you can send and receive."
						className="mb-0"
					/>

					<div className="w-full flex flex-col gap-4">
						<FundingOptionCard
							selected={selectedOption === "bootstrap"}
							title="Bootstrap"
							description="Build a service credit with the default liquidity provider and upgrade later."
							onSelect={() => setSelectedOption("bootstrap")}
							icon={
								<div
									style={{
										color:
											selectedOption === "bootstrap"
												? "white"
						...
						<FundingOptionCard
							selected={selectedOption === "connection"}
							title="Node connection"
							description="Enter a connection string (nprofile) to connect to a remote node over Nostr."
							onSelect={() => setSelectedOption("connection")}
							icon={
								<div
									style={{
										filter:
	...
	return (
		<button
			...
			className={cn(
				...
				selected
					? "bg-[var(--ion-color-secondary)] shadow-[var(--wallet-box-shadow)]"
					: "bg-[var(--app-surface)] shadow-[var(--wallet-box-shadow)] hover:brightness-[1.03]",
			)}
			onClick={onSelect}
		>
