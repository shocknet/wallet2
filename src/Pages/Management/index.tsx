...
import { SourceView, selectHealthySourceViews } from "@/State/scoped/backups/sources/selectors";
type OfferItemType = {
	title: string;
	value: string;
	type: string;
...
const Management = () => {
	...
	const [allValue, setAllValue] = useState<SourceView[]>(healthySources);

	const [display, setDisplay] = useState(0);
	const [rotation, setRotation] = useState(0);
	const [value, setValue] = useState(healthySources[0]);
	const [remainValues, setRemailValues] = useState<SourceView[]>([]);

	const [displayData, setDisplayData] = useState<SourceView>();
	...
	const dropdown = () => {
		const remainValues = allValue.filter((e) => e.sourceId !== value.sourceId);
	...
	const selectOption = (id: SourceView) => {
		dropdown();
		setValue(id);
	};
			...
			<div className="Offers">
				...
				<div className="Offers_item" onClick={dropdown}>
					<div className="selected_item">
						{value ? (
							<div className="item" key={value.sourceId}>

								<div className="Offers_from_value">{sourceDisplayName(value)}</div>
							</div>
						) : (
							<div></div>
						)}
						...
						>
							{display === 1 &&
								remainValues.map((item: SourceView) => {
									return (
										<div
											onClick={() => {
												selectOption(item);
										...
										>
											...
											<div className="Offers_from_value">{sourceDisplayName(item)}</div>
										</div>
									);
								})}
						</div>
