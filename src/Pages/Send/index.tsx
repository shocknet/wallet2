...
export const Send = () => {
  ...
  const paySource = useSelector((state) => state.paySource).map((e: any) => { return { ...e } });
  ...
  const decodeInvoice = async (input: string) => {
    ...
    try {
      const result = await (await getNostrClient({ pubkey: NOSTR_PUB_DESTINATION, relays: NOSTR_RELAYS })).DecodeInvoice({ invoice: input });
