...
import { IonButton, IonSkeletonText, IonText, IonInput, IonBadge, IonCard, IonCardContent, IonCardHeader, IonCardTitle, IonIcon } from "@ionic/react";
import { flashOutline, checkmarkCircle, closeCircle, timeOutline, walletOutline, alertCircle, cashOutline } from "ionicons/icons";
import { InvoiceSwapQuote, InvoiceSwapOperation } from "../../../Api/pub/autogenerated/ts/types";
...
function SwapCard({ swap, refundingOp, setRefundingOp, refundSatPerVByte, setRefundSatPerVByte, refundSwap, doSwap, busy }: SwapCardProps) {
			...
			<IonCardContent>
				...
				<div style={{ marginTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
					{!isRefunding && !quote.tx_id && status === 'unpaid_quote' && (
						<IonButton size="small" onClick={() => doSwap(quote.swap_operation_id)} disabled={busy}>
							Pay Quote
						</IonButton>
					)}
					...
					{isRefunding && (
						<>
							...
							<IonButton size="small" color="warning" onClick={refundSwap} disabled={busy}>
								Confirm Refund
							</IonButton>
...
export default function SubmarineSwaps() {
	...
	const { data, error, isFetching, refetch } = useListAdminInvoiceSwapsQuery({ sourceId });
	const [getQuotes, quotesMut] = useGetAdminInvoiceSwapQuotesMutation();
	const [paySwap, payState] = usePayAdminInvoiceSwapMutation();
	const [refundSwapMut, refundState] = useRefundAdminInvoiceSwapMutation();
	const busy = payState.isLoading || refundState.isLoading;
	...
	const requestQuote = async () => {
		try {
			const res = await getQuotes({ sourceId, amountSats }).unwrap();
	...
	const refundSwap = async () => {
		...
		try {
			await refundSwapMut({
				sourceId,
				sat_per_v_byte: refundSatPerVByte,
				swap_operation_id: refundingOp,
			}).unwrap();
			setRefundingOp(null);
		} catch (e) {
			toast.error((e as AppApiError).message || "Failed to refund swap");
		}
	};
	...
	const categorizedSwaps = useMemo(() => {
		if (!data) return [];
		return data.swaps.map(op => getSwapStatus(op, data.current_block_height));
	}, [data]);
					...
					<IonCardContent>
						<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
							...
							<IonButton
								...
								disabled={!amountSats || amountSats <= 0 || quotesMut.isLoading}
							>
							...
							<IonCardContent>
								<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
									...
									<IonButton
										...
										disabled={!satPerVByte || satPerVByte <= 0 || busy}
									>
										<IonIcon icon={flashOutline} slot="start" />
			...
			{loading ? (
				...
				<>
					<IonCard>
						...
						<IonCardContent>
							<IonText color="medium">
								Current Block: {data.current_block_height.toLocaleString()}
							</IonText>
							{data.swaps.length > 0 && (
								<div style={{ marginTop: '8px' }}>
									<IonText color="medium" style={{ fontSize: '0.9rem' }}>
										Total Operations: {data.swaps.length}
									</IonText>
								</div>
							)}
						</IonCardContent>
					...
					{/* No swaps message */}
					{data.swaps.length === 0 && quotes.length === 0 && (
						<IonCard>
