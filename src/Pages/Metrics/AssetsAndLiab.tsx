...
export const AssetsAndLiab = () => {
    ...
    const [assetsAndLiabilities, setAssetsAndLiabilities] = useState<AssetsAndLiabilitiesV2>({ liquidity_providers: [], lnds: [], users_balance: 0 });
    ...
    const initProviderHasMore = useCallback((data: AssetsAndLiabilitiesV2) => {
    ...
    const runFetch = useCallback(async (
        req: AssetsAndLiabilitiesReqV2,
        showFullLoading: boolean,
    ): Promise<AssetsAndLiabilitiesV2 | null> => {
        ...
        try {
            const res = await fetcher(
                ...
                [(client) => client.GetAssetsAndLiabilitiesV2(req)]
            );
            if (!res) return null;
            const [data] = res;
    ...
    }, [adminSource, dismissLoading, markNeedsUpgrade, presentLoading]);
    ...
    const loadMoreLnd = useCallback(async (pubkey: string) => {
        ...
        try {
            ...
            const mergedTracked: TrackedLndProviderV2 = {
    ...
    const loadMoreLp = useCallback(async (pubkey: string) => {
        ...
        try {
            ...
            const incomingTracked = incoming.tracked;
            const existingTracked = provider.tracked;
            const mergedTracked: TrackedLiquidityProviderV2 = {
    ...
    useEffect(() => {
        ...
        const fetchKey = `${sourceId}:ready`;
        if (autoFetchedForSourceIdRef.current === fetchKey) return;

        autoFetchedForSourceIdRef.current = fetchKey;
        void fetchDiscoveryRef.current();
    }, [checkingPubCapability, needsUpgrade, sourceId]);
