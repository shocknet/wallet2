...
type Props = { fetchPage: (page: number) => Promise<Uint8Array>, rpcMethod: string, methodMetrics: MetricsDataEntry<UsageMetric>, success: boolean, failure: boolean, http: boolean, nostr: boolean, selectedMetrics: SelectedMetrics }
...
export const UsageGraph = ({ fetchPage, rpcMethod, methodMetrics, failure, http, nostr, success, selectedMetrics }: Props) => {
    ...
    const [pagesData, setPagesData] = useState<Record<number, UsageMetric[]>>({})
    const { datasets, labels, from, to } = useMemo(() => {
        ...
        const datasets = datas.filter(({ data }) => data.length > 0).map((d) => ({
            label: d.label,
            data: d.data,
