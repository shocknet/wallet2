...
export const DebitRequestModal = () => {
	const price = useAppSelector((state) => state.usdToBTC);
	...
	const currentRequest = useMemo(() => {
		...
		if (debitRequests.length > 0) {
			...
			if (!view) return; // If the spend source was deleted ignore the request
	...
	}, [debitRequests, healthyNprofileViews])
	...
	const authroizeRequest = useCallback(async () => {
		...
		const invoice = currentRequest.request.debit.type === LiveDebitRequest_debit_type.INVOICE ? currentRequest.request.debit.invoice : undefined
		const r: DebitResponse_response = invoice && rules.length === 0 ? {
		...
		const res = await (await getNostrClient({ pubkey: source.lpk, relays: source.relays }, source.keys)).RespondToDebit({
		...
		dispatch(removeDebitRequest({ requestorNpub: currentRequest.request.npub, sourceId: currentRequest.source.sourceId }))
	...
	const removeCurrentRequest = useCallback(async () => {
		if (currentRequest) {
			const { source } = currentRequest
			await (await getNostrClient({ pubkey: source.lpk, relays: source.relays }, source.keys)).RespondToDebit({
				npub: currentRequest.request.npub,
				request_id: currentRequest.request.request_id,
				response: {
	...
	const banRequest = useCallback(async () => {
		if (!currentRequest) return;
		const { source } = currentRequest
		const res = await (await getNostrClient({ pubkey: source.lpk, relays: source.relays }, source.keys)).BanDebit({ npub: currentRequest.request.npub });
		if (res.status !== "OK") {
...
export const EditDebitModal = () => {
	const dispatch = useAppDispatch();
	const stateDebit = useAppSelector(state => state.modalsSlice.editDebit);
	const healthyNprofileViews = useAppSelector(selectHealthyNprofileViews);
	...
	const debitAuth = useMemo(() => {
		...
		const view = healthyNprofileViews.find(v => v.sourceId === stateDebit.sourceId);
		if (!view) return null;
		return { ...stateDebit, source: view }
	}, [stateDebit, healthyNprofileViews])
	...
	const updateDebitAuth = useCallback(async () => {
		if (debitAuth) {
			...
			const res = await (await getNostrClient({ pubkey: source.lpk, relays: source.relays }, source.keys)).EditDebit({ authorize_npub: debitAuth.npub, rules });
			if (res.status !== "OK") {
	...
	const resetRequest = useCallback(async () => {
		if (debitAuth) {
			const { source } = debitAuth;
			const res = await (await getNostrClient({ pubkey: source.lpk, relays: source.relays }, source.keys)).ResetDebit({ npub: debitAuth.npub });
			if (res.status !== "OK") {
