...
function OfferInfoModal({ sourceId, initialOffer, dismiss }: OfferInfoModalProps) {
	...
	const {
		...
		{ sourceId, offerId: initialOffer?.offer_id ?? "" },
		{ skip: !initialOffer },
	);
	...
	const invoicesBusy = invoiceLoading || (isFetching && invoices.length === 0);
	...
	const doSave = async () => {
		const offer = { ...draft };
		const result = isNew
			? await addOffer({ sourceId, offer })
			: await updateOffer({ sourceId, offer });
		if (result.error) {
			showToast({
				message: result.error.message ?? "Failed to save offer",
				color: "danger",
			});
			return;
		}
