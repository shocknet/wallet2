...
import { isAppInstall } from "@/State/identitiesRegistry/helpers/identityNostrApi";
...
export class SanctumSession {
	...
	private readonly opts: Required<Omit<SanctumSessionOptions,
		"onRetryScheduled" | "sendOnOpen" | "onDeprecation"
	>> & {
		sendOnOpen: Record<string, string | number | boolean>;
		onRetryScheduled?: (attempt: number, delayMs: number) => void;
		onDeprecation?: (notice: DeprecationNotice) => void;
	...
	private openSocket() {
		...
		ws.onmessage = event => {
			...
			if (parsed.deprecation && isAppInstall() && this.opts.onDeprecation) {
				this.opts.onDeprecation(parsed.deprecation as DeprecationNotice);
			}
