...
export const sendPaymentThunk = (
	...
	return async (dispatch, getState) => {
		...
		const getOpKey = (optimisticId: string) => makeKey(sourceId, optimisticId)
		...
		const payInvoice = async (parsedInvoice: ParsedInvoiceInput, optimisticOperationId: string, optimisticOperation: SourceOptimsiticInvoice) => {
			try {
				...
				handlePaymentSuccess(dispatch, amount, selectedSource, optimisticOperationId, payInvoiceReponseToUserOperation(res, optimisticOperation), showToast);
		...
		switch (parsedInput.type) {
			case (InputClassification.LN_INVOICE): {
				const operationId = parsedInput.data; // use the invoice as the operationId for the optimistic operation entry
				const opKey = getOpKey(operationId);
				...
				payInvoice(parsedInput, operationId, optimsticOperation);
				return;
			}
			...
			case InputClassification.LNURL_PAY: {
				...
				const operationId = parsedInvoice.data; // use the invoice as the operationId for the optimistic operation entry
				const opKey = getOpKey(operationId);
				...
				payInvoice(parsedInvoice, operationId, optimsticOperation);
				return;
			}
