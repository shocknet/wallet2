...
const migrations = {
	1: (state: SourcesState) => {
		for (const id of state.docs.ids) {
			const src = state.docs.entities[id];
			if (!src) continue;
			if (src.draft.type !== SourceType.NPROFILE_SOURCE) continue;

			const relays = src.draft.relays;
			const keys = Object.keys(relays);

			const next: typeof relays = {};

			const winnerFromSlash: Record<string, boolean> = {};

			for (const r of keys) {
				const relayData = relays[r];
				const newR = normalizeWsUrl(r);
				const isSlash = r.endsWith("/");

				if (!(newR in next)) {
					next[newR] = relayData;
					winnerFromSlash[newR] = isSlash;
					continue;
				}


				if (winnerFromSlash[newR] && !isSlash) {
					next[newR] = relayData;
					winnerFromSlash[newR] = false;
				}
			}

			src.draft.relays = next;
		}
