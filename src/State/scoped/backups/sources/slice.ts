...
import { SourceActualOperation, SourceOperation, SourceOptimsticOperation } from "./history/types";
...
export const sourcesSlice = createSlice({
	...
	reducers: {
		 ...
		 * UI can hide if draft.deleted.value === true.
		...
		mergePage(
			...
			for (const op of operations) {
				...
				const existed = !!state.history.ops.entities[key];
				const sourceOperation = userOperationToSourceOperation(op, sourceId, state.history.ops.entities[key])
		...
		/**
		 * Ingest a single live operation (websocket/nostr push).
		 * Behaves like mergePage for a 1-op page.
		 */
		ingestLive(
			state,
			...
			const existed = !!state.history.ops.entities[key];
			const sourceOperation = userOperationToSourceOperation(operation, sourceId, state.history.ops.entities[key])
			...
			if (!existed) state.history.newPaymentsCount += 1;
		},

		/**
		 * Add an optimistic op for a source (outbound send).
		 * Only adds if the optimistic key doesn’t already exist under that source.
		 */
		addOptimistic(
			state,
		...
		/**
		 * Update/replace an optimistic op with a new op (often real op with new ID).
		 * - If the operationId changes, we delete the old key and insert the new one.
		 */
		replaceOptimistic(
			state,
			action: PayloadAction<{
		...
		/**
		 * Remove an optimistic op by its client-side operationId.
		 */
		removeOptimistic(
			state,
		...
		/**
		 * Update just the memo/note for a given op.
		 */
		setOperationNote(
			state,
			...
			if (!op) return;
			// preserve optimistic flag/type union
			opsAdapter.updateOne(state.history.ops, { id: key, changes: { memo: note } as Partial<SourceOperation> });
		},

		/**
		 * Reset cursors and clear lists (keeping entity cache is optional;
		 * here we clear lists & entities for each source so refetches get clean pages).
		 */
		...
		/**
		 * Clear a single source’s history (for cascade deletes or manual reset).
		 */
		...
		/**
		 * Clear multiple sources at once (bulk cascade).
		 */
		...
		/**
		 * Optionally set/replace cursor for a source (useful for manual tweaks or migrations).
		 */
		setCursor(
			state,
		...
		/**
		 * Bump/reset the UI counter that shows newly discovered payments.
		 */
...
export const selectIsSourceDocDirty = (state: RootState, sourceId: string) => !!docsSelectors.selectById(state, sourceId)?.draft
