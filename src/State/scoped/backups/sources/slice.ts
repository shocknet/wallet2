...
const migrations = {
	...
	4: (state: SourcesState): SourcesState => {
		if (!state) return state;

		delete (state as SourcesState & { beaconProbe?: unknown }).beaconProbe;

		const metadata = state.metadata as SourcesMetadataState & { beaconStaleMs?: number };
		if (metadata) {
			delete metadata.beaconStaleMs;
			for (const id of metadata.ids) {
				const m = metadata.entities[id] as (SourceMetadata & {
					beaconName?: unknown;
					beaconAvatarUrl?: unknown;
					beaconFees?: unknown;
					beaconNextRelay?: unknown;
					lastSeenAtMs?: unknown;
				}) | undefined;
				if (!m) continue;
				delete m.beaconName;
				delete m.beaconAvatarUrl;
				delete m.beaconFees;
				delete m.beaconNextRelay;
				delete m.lastSeenAtMs;
			}
		}
