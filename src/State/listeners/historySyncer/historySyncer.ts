...
import { TaskAbortError, TaskResult } from "@reduxjs/toolkit";
import { UserOperation } from "@/Api/pub/autogenerated/ts/types";
import { emptyCursor, getPaymentStateResponseToUserOperation, isInFlightOutgoingInvoice, parseOperationsResponse, } from "@/State/scoped/backups/sources/history/helpers";
		...
		(add) =>
			add({
				...
				effect: async (action, listenerApi) => {
					...
					const fetchUserInfoTask = listenerApi.fork(async forkApi => {
						try {
							...
							if (!forkApi.signal.aborted) {
								listenerApi.dispatch(
									sourcesActions.applyUserInfo({
										sourceId,
										balance: infoRes.balance as Satoshi,
										maxWithdrawable: infoRes.max_withdrawable as Satoshi,
										ndebit: infoRes.ndebit,
										noffer: infoRes.noffer,
										nmanage: infoRes.nmanage,
										topicId: infoRes.topic_id,
									})
								);
							}
						} catch (err) {
					...
					const tasks = Promise.all([fetchUserInfoTask.result, fetchTransactionsTask.result]);
		...
		(add) =>
			add({
				...
				effect: (action, listenerApi) => {
					...
					for (const sourceId of sourceIdsThatBecameFresh(lpk, curr, prev)) {
						startFetchForSource(sourceId, listenerApi.dispatch);
					}
				},
			}),
		// On app boot or resume, fetch payment state for in-flight outgoing invoices
		(add) =>
			add({
				predicate: (action) => {
				...
				effect: async (_, listenerApi) => {
					...
					const inFlight = allOperations.filter(isInFlightOutgoingInvoice);
					...
					const task = listenerApi.fork(async forkApi => {
						...
						inFlight.map(async o => {
							...
							const log = dLogger.withContext({
								procedure: "in-flight outgoing invoice check",
							...
							try {
								...
								listenerApi.dispatch(
									sourcesActions.ingestLive({
										sourceId,
										operation: getPaymentStateResponseToUserOperation(res, invoice),
									})
								);
