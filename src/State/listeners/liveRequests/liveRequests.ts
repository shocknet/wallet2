...
import { UserOperationType } from "@/Api/pub/autogenerated/ts/types";
...
const subscribeToStreams = (
	...
	client.GetLiveUserOperations((newOp) => {
		if (newOp.status === "OK") {
			...
			if (op.type !== UserOperationType.OUTGOING_INVOICE) {
				listenerApi.dispatch(
					sourcesActions.ingestLive({ sourceId, operation: op }) // If it's an outgoing invoice payment the reducer will ignore it.
				);
				listenerApi.dispatch(
			...
			if (op.paidAtUnix > 0) {
				listenerApi.dispatch(
					sourcesActions.replaceOptimistic({
						sourceId,
						oldOperationId: op.identifier, // the invoice, which the optimistic operation uses as operationId
						operation: op
					})
				);
