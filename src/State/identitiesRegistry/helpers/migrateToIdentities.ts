...
import { initialState as paySourcesInitialState } from "@/State/Slices/paySourcesSlice";
import { initialState as spendSourcesInitialState } from "@/State/Slices/spendSourcesSlice";
import { findReducerMerger } from "@/State/store/store";
import { GeneralShard, ShardsTagsRecord } from "@/State/types";
import { utils } from "nostr-tools";

type SourceToMigrate = SpendFrom | PayTo;
export async function getMigratedSources(ext: IdentityNostrApi) {
	// Get sources from local state (local storage)
	const sources: SourceToMigrate[] = Object.values(paySourcesInitialState.sources).concat(Object.values(spendSourcesInitialState));

	// Get sources from legacy remote backups
	const remoteSources = await getLegacyRemoteBackup(ext);
	const allSources = sources.concat(remoteSources);

	// migration all to source docs and dedupe
	const docs = await migrateSourcesToDocs(allSources);
...
export async function getLegacyRemoteBackup(ext: IdentityNostrApi): Promise<SourceToMigrate[]> {
