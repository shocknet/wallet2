...
import { maybeShowDeprecationNotice } from "@/lib/deprecationNotices";
import { DeprecationNotice } from "@/Api/keylink/autogenerated/ts/types";
import { Capacitor } from "@capacitor/core";
import { isPlatform } from "@ionic/react";
import { getExtentionsWithRetries } from "@/helpers/nip07Extention";
...
export const isAppInstall = () => isPlatform("pwa") || isPlatform("ios") || isPlatform("android");
const maybeShowKeylinkDeprecation = (r: { deprecation?: DeprecationNotice }) => {
	if (r.deprecation && isAppInstall()) {
		maybeShowDeprecationNotice(r.deprecation);
	}
};
...
export async function getSanctumIdentityApi({ pubkey, accessToken }: { pubkey?: string, accessToken: string }): Promise<IdentityNostrApi> {
	...
	const api: IdentityNostrApi = {
		decrypt: (pubkey, ciphertext) => keyLinkClient.LegacyNip44Decrypt({ pubkey, ciphertext }).then((r) => { if (r.status !== 'OK') { handleSanctumGeneralErrors(r.reason); throw new Error(r.reason); } else { maybeShowKeylinkDeprecation(r); return r.plaintext } }),
		encrypt: (pubkey, plaintext) => keyLinkClient.LegacyNip44Encrypt({ pubkey, plaintext }).then((r) => { if (r.status !== 'OK') { handleSanctumGeneralErrors(r.reason); throw new Error(r.reason); } else { maybeShowKeylinkDeprecation(r); return r.ciphertext } }),
		getPublicKey: () => keyLinkClient.LegacyGetNostrPubKey().then((r) => { if (r.status !== 'OK') { handleSanctumGeneralErrors(r.reason); throw new Error(r.reason); } else { maybeShowKeylinkDeprecation(r); return r.pubkey } }),
		signEvent: (event) => keyLinkClient.LegacySignNostrEvent({ usignedEvent: JSON.stringify(event) }).then((r) => { if (r.status !== 'OK') { handleSanctumGeneralErrors(r.reason); throw new Error(r.reason); } else { maybeShowKeylinkDeprecation(r); return JSON.parse(r.signedEvent) } }),
		getRelays: () => keyLinkClient.LegacyGetNostrRelays().then((r) => { if (r.status !== 'OK') { handleSanctumGeneralErrors(r.reason); throw new Error(r.reason); } else { maybeShowKeylinkDeprecation(r); return r.relays } })
	}
