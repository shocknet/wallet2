...
import { docsSelectors, selectCursorForSource, sourcesActions } from "@/State/scoped/backups/sources/slice";
	...
	startListening({
		...
		effect: async (_, listenerApi) => {
			...
			const subForNewSources = listenerApi.dispatch(
				addAppListener({
					predicate: (action) => sourcesActions.applyRemoteSource.match(action) || sourcesActions._createDraftDoc.match(action),
					effect: (action, listenerApi) => {
						const { sourceId } = action.payload;

						if (sourcesActions.applyRemoteSource.match(action)) {
							const source = docsSelectors.selectById(listenerApi.getState(), sourceId)
