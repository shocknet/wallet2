...
const SOURCE_NS = "shockwallet:source";
...
function processRemoteIdentityDoc(doc: DocBase, dispatch: AppThunkDispatch): IdentityDocV0 | null {
	let migrationResult;
...
function processRemoteSourceDoc(doc: DocBase, dispatch: AppThunkDispatch): SourceDocV0 | null {
	let migrationResult;
	...
	if (migrationResult === "AHEAD") {
		...
		const docParseResult = SourceDocV0Schema.safeParse(migrationResult);
		if (!docParseResult.success) {
			return null;
		}
		dispatch(sourcesActions.applyRemoteSource({ remote: docParseResult.data }));
		return docParseResult.data
	}
}
export function processRemoteDoc(doc: unknown, dispatch: AppThunkDispatch): IdentityDocV0 | SourceDocV0 | null {
